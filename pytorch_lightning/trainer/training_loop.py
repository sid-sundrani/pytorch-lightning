...
class TrainLoop:
    ...
    def on_train_epoch_start(self, epoch):
        ...
        if reload_dl_every_n_epochs and (not epoch % reload_dl_every_n_epochs):
            self.trainer.should_reload_dl_epoch = True
            self.trainer.reset_train_dataloader(model)
        else:
            self.trainer.should_reload_dl_epoch = False
    ...
    def reset_train_val_dataloaders(self, model):
        if not self.trainer.reload_dataloaders_every_n_epochs:
            self.trainer.reset_train_dataloader(model)

        if self.trainer.val_dataloaders is None and not self.trainer.reload_dataloaders_every_n_epochs:
